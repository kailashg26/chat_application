...
int addGreetingResponse(char* msg_buf, int size)
{
    std::cout << "In addGreetingResponse() Original message: " << msg_buf << std::endl;
    
    int x;
    scanf("%d", &x);

    if (strstr(msg_buf, "hello") != NULL)
    {
        ...
        std::cout << "Special response (" << msg.length() << "): " << msg_buf << std::endl;
        size = msg.length();
    }

    std::cout << "Print statement for debug\n";
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    // If there is some bytes
      ...
      bytes = addGreetingResponse(msg_buf, bytes);
